...
public class SentimentAnalysis {
    ...
    public static class SentimentSplit extends Mapper<Object, Text, Text, IntWritable> {
        ...
        public Map<String, String> emotionDic = new HashMap<String, String>(); // setup the dictionary

        @Override
        public void setup(Context context) throws IOException{
            Configuration configuration = context.getConfiguration();
            String dicName = configuration.get("dictionary", ""); // dynamic get the args
        ...
        public void map(Object key, Text value, Context context) throws IOException, InterruptedException {
            // key can mark the position of current input
            // value seperated by '\n'

            String fileName = ((FileSplit) context.getInputSplit()).getPath().getName();
            String line = value.toString().trim();
            String[] words = line.split("\\s+"); // 1. split by space
            for (String word: words) { // 2. look up word in dictionary (already defined in emotionCategory.txt and loaded in setup)
                if (emotionDic.containsKey(word.trim().toLowerCase())) {
                    context.write(new Text(fileName + "\t" + emotionDic.get(word.toLowerCase())), new IntWritable(1)); // 3. extract emotion and write out
                }
            }
